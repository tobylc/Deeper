...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // PRODUCTION FIX: Database integrity check and repair for turn synchronization
  app.post("/api/conversations/repair-turns", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims?.sub || req.user.id;
      const currentUser = await storage.getUser(userId);
      ...
      console.log(`[TURN_REPAIR] Starting turn repair for user: ${currentUser.email}`);
      
      // Get all conversations where this user is a participant
      const userConversations = await storage.getConversationsByEmail(currentUser.email);
      let repairedCount = 0;
      
      for (const conversation of userConversations) {
        // Check if turn state is valid
        const messages = await storage.getMessagesByConversationId(conversation.id);
        
        // Determine correct turn based on message count and participants
        let correctTurn: string;
        
        if (messages.length === 0) {
          // Empty conversations: inviter (participant1) should have the turn
          correctTurn = conversation.participant1Email;
        } else {
          // Non-empty conversations: turn should alternate based on last message
          const lastMessage = messages[messages.length - 1];
          correctTurn = lastMessage.senderEmail === conversation.participant1Email 
            ? conversation.participant2Email 
            : conversation.participant1Email;
        }
        
        // Update turn if it's incorrect
        if (conversation.currentTurn !== correctTurn) {
          console.log(`[TURN_REPAIR] Fixing conversation ${conversation.id}: ${conversation.currentTurn} -> ${correctTurn}`);
          await storage.updateConversationTurn(conversation.id, correctTurn);
          repairedCount++;
          
          // Send WebSocket notifications to both participants about the turn correction
          try {
            const { getWebSocketManager } = await import('./websocket');
            const wsManager = getWebSocketManager();
            if (wsManager) {
              ...
              wsManager.notifyConversationUpdate(conversation.participant1Email, notificationData);
              if (conversation.participant1Email !== conversation.participant2Email) {
                wsManager.notifyConversationUpdate(conversation.participant2Email, notificationData);
              }
            }
          } catch (error) {
            console.error('[WEBSOCKET] Failed to send turn repair notification:', error);
          }
        }
      }
      
      console.log(`[TURN_REPAIR] Completed: repaired ${repairedCount} conversations for ${currentUser.email}`);
      
      res.json({ 
        success: true,
        repairedConversations: repairedCount,
        totalConversations: userConversations.length,
        message: `Turn synchronization repaired for ${repairedCount} conversations`
      });
    } catch (error) {
      console.error("Turn repair error:", error);
      res.status(500).json({ message: "Failed to repair turn synchronization" });
    }
  });
